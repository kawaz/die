...
| `--help` | help を stderr に出して exit 1。`--` より前で指定する必要あり。`--` の後ろに置くと ARG (literal `--help`) として扱う。 |
...
- ARGS 空 (= `--` なし) + stdin が **TTY** → help を stderr に出して exit 1。
...
1. `die` 単独 + stdin TTY → help を stderr に出す (= exit 1)。
2. `--help` option を `--` より前に指定 → 同じ help を出す (= exit 1)。stdin の状態に依存しない。

`--help` を導入しつつも DR-0001 の「`--` 以降は何でも safe に渡せる」property は維持: `die -- --help` は literal "--help" を stderr に echo する (= ARG 扱い)。詳細は [DR-0008](./decisions/DR-0008-stdin-tty-routing-and-help-option.md)。
