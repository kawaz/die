...
| `--help` | help を stderr に出して **exit 0** (meta query)。`--` より前で指定する必要あり。`--` の後ろに置くと ARG (literal `--help`) として扱う。 |
| `--version` | `die <version>` を stderr に出して **exit 0** (meta query)。`--` より前で指定する必要あり。`--` の後ろに置くと ARG として扱う。 |
...
- **出力先**: 常に stderr
- **exit code**: die 本来動作 (= ARG / stdin / bare-TTY help fallback / usage error) は **1**。**明示の meta query (= `--help` / `--version`) のみ 0**。詳細は [DR-0009](./decisions/DR-0009-exit-code-policy-and-version-option.md)。
- **`--` 必須**: opts と ARGS の境界マーカー。`die foo` (= `--` なし) は syntax error
- **環境変数**: 一切採用しない (= 挙動はすべて argv に閉じる)

### stdin 経由

経路は `--` の有無と (`--` なしの時) stdin が TTY か否かで決まる:

- ARGS あり (= `--` あり) → ARG 経路。stdin が pipe でも無視。
- ARGS 空 (= `--` なし) + stdin が **TTY でない** → stdin を stderr に転送。「TTY でない」は anonymous pipe / 名前付き FIFO / regular file / char device (`/dev/null`, `/dev/zero` 等) / socket (process substitution) / block device 全部を含む。`/dev/null` は空入力として転送 → normalize ルールで `\n` 補完。
- ARGS 空 (= `--` なし) + stdin が **TTY** → help を stderr に出して exit 1 (= bare-TTY help fallback、usage error カテゴリ。明示 `--help` 要求 = exit 0 とは区別)。

TTY 判定の実装 (詳細は [DR-0008](./decisions/DR-0008-stdin-tty-routing-and-help-option.md) と [findings/2026-06-28-tty-detection-cross-os.md](./findings/2026-06-28-tty-detection-cross-os.md)):

- POSIX: `isatty(3)` (= `ioctl(TCGETS)` / `TIOCGETA`)。
- Windows: std handle に対する `GetConsoleMode()`。MSVCRT `_isatty()` は意図的に使わない (= NUL device を TTY と誤判定する既知挙動)。
- Cygwin / MSYS2 / Git Bash pty: 内部 named pipe の名前パターン (`\msys-…-ptyN-…`) を `NtQueryObject` で取得 → 照合。Git Bash プロンプトで `die` 裸打ちした場合も help が出る。
...
### help / version meta query の出し方

同じ help text を出す経路が 2 つ:

1. `die` 単独 + stdin TTY → help を stderr に出す (= **exit 1**、bare-TTY fallback、usage error)。
2. `--help` option を `--` より前に指定 → 同じ help を出す (= **exit 0**、明示 meta query)。stdin の状態に依存しない。

`--version` は明示 `--help` と同型: `die --version` → `die <X.Y.Z>\n` を stderr に出して exit 0。

DR-0001 の「`--` 以降は何でも safe に渡せる」property は維持: `die -- --help` / `die -- --version` は literal を stderr に echo する (= ARG 扱い、exit 1)。詳細は [DR-0008](./decisions/DR-0008-stdin-tty-routing-and-help-option.md) と [DR-0009](./decisions/DR-0009-exit-code-policy-and-version-option.md)。
...
実装は **Zig** (DR-0007)。配布は `brew install kawaz/tap/die`。並行実装フェーズ (Go / Rust / MoonBit / Zig 比較、DR-0003) の commit 系列は `archive/multi-impl-comparison` bookmark に保存。詳細な計測根拠は `docs/findings/2026-06-27-language-comparison.md`。
